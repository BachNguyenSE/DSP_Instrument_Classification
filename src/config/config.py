# src/config.py (moved to config/config.py)
...
FEATURE_DIR: Path = DATA_DIR / "features"  # Không còn sử dụng
PROCESSED_DIR: Path = DATA_DIR / "processed"  # Không còn sử dụng
...
# Tạo thư mục nếu chưa tồn tại (bỏ features, processed)
for d in [RAW_DATA_DIR, MODEL_DIR, TEST_DIR]:
